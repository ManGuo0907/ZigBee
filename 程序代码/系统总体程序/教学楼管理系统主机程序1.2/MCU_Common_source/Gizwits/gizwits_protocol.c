...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagservo1)
    {
        info->event[info->num] = EVENT_servo1;
        info->num++;
        dataPoints->valueservo1 = gizStandardDecompressionValue(servo1_BYTEOFFSET,servo1_BITOFFSET,servo1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagledflag1)
    {
        info->event[info->num] = EVENT_ledflag1;
        info->num++;
        dataPoints->valueledflag1 = gizStandardDecompressionValue(ledflag1_BYTEOFFSET,ledflag1_BITOFFSET,ledflag1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagairflag1)
    {
        info->event[info->num] = EVENT_airflag1;
        info->num++;
        dataPoints->valueairflag1 = gizStandardDecompressionValue(airflag1_BYTEOFFSET,airflag1_BITOFFSET,airflag1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagservo2)
    {
        info->event[info->num] = EVENT_servo2;
        info->num++;
        dataPoints->valueservo2 = gizStandardDecompressionValue(servo2_BYTEOFFSET,servo2_BITOFFSET,servo2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagledflag2)
    {
        info->event[info->num] = EVENT_ledflag2;
        info->num++;
        dataPoints->valueledflag2 = gizStandardDecompressionValue(ledflag2_BYTEOFFSET,ledflag2_BITOFFSET,ledflag2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagairflag2)
    {
        info->event[info->num] = EVENT_airflag2;
        info->num++;
        dataPoints->valueairflag2 = gizStandardDecompressionValue(airflag2_BYTEOFFSET,airflag2_BITOFFSET,airflag2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagdata)
    {
        ...
        info->num++;
        gizMemcpy((uint8_t *)dataPoints->valuedata,issuedData->attrVals.valuedata,sizeof(issuedData->attrVals.valuedata));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueservo1 != cur->valueservo1)
    {
        GIZWITS_LOG("valueservo1 Changed\n");
        ret = 1;
    }
    if(last->valueledflag1 != cur->valueledflag1)
    {
        GIZWITS_LOG("valueledflag1 Changed\n");
        ret = 1;
    }
    if(last->valueairflag1 != cur->valueairflag1)
    {
        GIZWITS_LOG("valueairflag1 Changed\n");
        ret = 1;
    }
    if(last->valueservo2 != cur->valueservo2)
    {
        GIZWITS_LOG("valueservo2 Changed\n");
        ret = 1;
    }
    if(last->valueledflag2 != cur->valueledflag2)
    {
        GIZWITS_LOG("valueledflag2 Changed\n");
        ret = 1;
    }
    if(last->valueairflag2 != cur->valueairflag2)
    {
        GIZWITS_LOG("valueairflag2 Changed\n");
        ret = 1;
    }
    ...
    if(last->valuetemp1 != cur->valuetemp1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetemp1 Changed\n");
            ret = 1;
        }
    }
    if(last->valuehumi1 != cur->valuehumi1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuehumi1 Changed\n");
            ret = 1;
        }
    }
    if(last->valuesignnumber != cur->valuesignnumber)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesignnumber Changed\n");
            ret = 1;
        }
    }
    if(last->valuerealnumber != cur->valuerealnumber)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuerealnumber Changed\n");
            ret = 1;
        }
    }
    if(last->valueinounumber != cur->valueinounumber)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueinounumber Changed\n");
            ret = 1;
        }
    }
    if(last->valuetemp2 != cur->valuetemp2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetemp2 Changed\n");
            ret = 1;
        }
    }
    if(last->valuehumi2 != cur->valuehumi2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuehumi2 Changed\n");
            ret = 1;
        }
    }
    if(last->valuesignnumber2 != cur->valuesignnumber2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesignnumber2 Changed\n");
            ret = 1;
        }
    }
    if(last->valuerealnumber2 != cur->valuerealnumber2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuerealnumber2 Changed\n");
            ret = 1;
        }
    }
    if(last->valueinounumber2 != cur->valueinounumber2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueinounumber2 Changed\n");
            ret = 1;
        }
    }
    if(last->valuedust1 != cur->valuedust1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedust1 Changed\n");
            ret = 1;
        }
    }
    ...
    if(last->valuedust2 != cur->valuedust2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedust2 Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(servo1_BYTEOFFSET,servo1_BITOFFSET,servo1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueservo1);
    gizStandardCompressValue(ledflag1_BYTEOFFSET,ledflag1_BITOFFSET,ledflag1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueledflag1);
    gizStandardCompressValue(airflag1_BYTEOFFSET,airflag1_BITOFFSET,airflag1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueairflag1);
    gizStandardCompressValue(servo2_BYTEOFFSET,servo2_BITOFFSET,servo2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueservo2);
    gizStandardCompressValue(ledflag2_BYTEOFFSET,ledflag2_BITOFFSET,ledflag2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueledflag2);
    gizStandardCompressValue(airflag2_BYTEOFFSET,airflag2_BITOFFSET,airflag2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueairflag2);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valuetemp1 = gizY2X(temp1_RATIO,  temp1_ADDITION, dataPoints->valuetemp1); 
    devStatusPtr->valuehumi1 = gizY2X(humi1_RATIO,  humi1_ADDITION, dataPoints->valuehumi1); 
    devStatusPtr->valuesignnumber = gizY2X(signnumber_RATIO,  signnumber_ADDITION, dataPoints->valuesignnumber); 
    devStatusPtr->valuerealnumber = gizY2X(realnumber_RATIO,  realnumber_ADDITION, dataPoints->valuerealnumber); 
    devStatusPtr->valueinounumber = gizY2X(inounumber_RATIO,  inounumber_ADDITION, dataPoints->valueinounumber); 
    devStatusPtr->valuetemp2 = gizY2X(temp2_RATIO,  temp2_ADDITION, dataPoints->valuetemp2); 
    devStatusPtr->valuehumi2 = gizY2X(humi2_RATIO,  humi2_ADDITION, dataPoints->valuehumi2); 
    devStatusPtr->valuesignnumber2 = gizY2X(signnumber2_RATIO,  signnumber2_ADDITION, dataPoints->valuesignnumber2); 
    devStatusPtr->valuerealnumber2 = gizY2X(realnumber2_RATIO,  realnumber2_ADDITION, dataPoints->valuerealnumber2); 
    devStatusPtr->valueinounumber2 = gizY2X(inounumber2_RATIO,  inounumber2_ADDITION, dataPoints->valueinounumber2); 

    devStatusPtr->valuedust1 = exchangeBytes(gizY2X(dust1_RATIO,  dust1_ADDITION, dataPoints->valuedust1)); 
    devStatusPtr->valuelight1 = exchangeBytes(gizY2X(light1_RATIO,  light1_ADDITION, dataPoints->valuelight1)); 
    devStatusPtr->valuedust2 = exchangeBytes(gizY2X(dust2_RATIO,  dust2_ADDITION, dataPoints->valuedust2)); 
    devStatusPtr->valuelight2 = exchangeBytes(gizY2X(light2_RATIO,  light2_ADDITION, dataPoints->valuelight2)); 



    gizMemcpy((uint8_t *)devStatusPtr->valuedata,(uint8_t *)&dataPoints->valuedata,sizeof(dataPoints->valuedata));
    return 0;
}
