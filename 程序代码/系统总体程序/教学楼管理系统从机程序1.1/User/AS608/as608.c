  ...
  while(key_num != 1)
  {
    ...
    if(ensure == 0x00) //获取图像成功
    {
      ...
      if(ensure == 0x00) //生成特征成功
      {
        ...
        //  sprintf(str, " ID:%d 得分:%d ", seach.pageID, seach.mathscore);
         // OLED_ShowCH(0, 4, (u8*)str);
					OLED_ShowInt(0,4,seach.pageID,2);
					admID = seach.pageID;
          DelayMs(1500);
          DelayMs(1500);
        }
...
//		if(signinnum==figernum)
//		{
//			OLED_CLS();
//			OLED_ShowCH(0, 0, "全部签到完成");
//			DelayMs(1500);
//		}
  }
	for(i=0;i<figernum;i++)
	{
		if(tag[i]==1)
			signinnum++;
	}
  OLED_CLS();
	OLED_ShowCH(0, 0,"需签到人数");
	OLED_ShowInt(45,2,(float)figernum,2);
	OLED_ShowCH(0, 4,"实际签到人数");
	OLED_ShowInt(45,6,signinnum,2);
	Node_data1.relnum=figernum;
	Node_data1.signnum=signinnum;
	DelayMs(1500);
