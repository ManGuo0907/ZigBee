...
{
   i2c_Start();                  //起始信号
   I2C_WriteByte(BHAddWrite);   //发送设备地址+写信号
...
void bh_data_send(u8 command)
{
    do{
    i2c_Start();                      //iic起始信号
    I2C_WriteByte(BHAddWrite);       //发送器件地址
    }while(i2c_WaitAck());           //等待从机应答
    I2C_WriteByte(command);          //发送指令
    i2c_WaitAck();                   //等待从机应答
    i2c_Stop();                       //iic停止信号
}
...
u16 bh_data_read(void)
{
	u16 buf;
	i2c_Start();                       //iic起始信号
	I2C_WriteByte(BHAddRead);         //发送器件地址+读标志位
	i2c_WaitAck();                     //等待从机应答
	buf=i2c_ReadByte(); 
	i2c_Ack();	                    //读取数据
