...
//输出粉尘浓度，单位：ug/m3.
//正常数值在150左右，有烟雾或者检查孔中由异物数值能达到200、300、400及以上。
//常见数值——正常157，异物456，烟雾340。
float GP2Y10_Value(void)
{
	...
	value=(v_fen*0.1767f+0.1564)*1000;
...
float GP2Y10_Value(void)
{
	float v;
	GP2Y10_LED_ON();
	SysTick_Delay_Us(GP2Y10_SAMP_TIME);
//	v=ADC_ConvertedValue * 3.3f /4096;
	v = LIMIT(ADC_ConvertedValue * 3.3f / 4095, 0.5846f, 3.5229f);
	SysTick_Delay_Us(19);
