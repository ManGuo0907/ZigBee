...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flaggear)
    {
        ...
        info->num++;
        dataPoints->valuegear = gizStandardDecompressionValue(gear_BYTEOFFSET,gear_BITOFFSET,gear_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuetotal_num != cur->valuetotal_num)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetotal_num Changed\n");
            ret = 1;
        }
    }
    if(last->valuereal_num != cur->valuereal_num)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuereal_num Changed\n");
            ret = 1;
        }
    }
    if(last->valuedust != cur->valuedust)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedust Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(gear_BYTEOFFSET,gear_BITOFFSET,gear_LEN,(uint8_t *)devStatusPtr,dataPoints->valuegear);
    ...
    devStatusPtr->valuetotal_num = gizY2X(total_num_RATIO,  total_num_ADDITION, dataPoints->valuetotal_num); 
    devStatusPtr->valuereal_num = gizY2X(real_num_RATIO,  real_num_ADDITION, dataPoints->valuereal_num); 

    devStatusPtr->valuedust = exchangeBytes(gizY2X(dust_RATIO,  dust_ADDITION, dataPoints->valuedust)); 
