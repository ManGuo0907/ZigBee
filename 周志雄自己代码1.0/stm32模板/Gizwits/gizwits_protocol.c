...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagrgb)
    {
        info->event[info->num] = EVENT_rgb;
        info->num++;
        dataPoints->valuergb = gizStandardDecompressionValue(rgb_BYTEOFFSET,rgb_BITOFFSET,rgb_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuergb != cur->valuergb)
    {
        GIZWITS_LOG("valuergb Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    if(dataPoints->valuergb >= rgb_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuergb Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(rgb_BYTEOFFSET,rgb_BITOFFSET,rgb_LEN,(uint8_t *)devStatusPtr,dataPoints->valuergb);  
    }
